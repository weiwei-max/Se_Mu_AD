...
# myadd
from sel_update import MTLoss_affinity, form_task_affinity, create_config

# dwdwdw
# from aligned_mtl import AlignedMTLOptimizer
# from aligned_mtl_ub import AlignedMTLUB
# from team_code.aligned_solver import ProcrustesSolver
...
def main():
  ...
  config = GlobalConfig() # 加载默认配置 GlobalConfig用于存储训练过程中的默认配置 /team_code/config.py
  ...
  ncpus_per_node = args.cpu_cores # CPU核心数
  num_workers = int(ncpus_per_node / ngpus_per_node) # 线程数
  ...
  config.initialize(**vars(args)) # config.initialize 加载训练和验证数据
  ...
  if config.use_plant:
    ...
    config.detailed_loss_weights['loss_forcast'] = 0.0 # use
  ...
  if config.lidar_seq_len == 1 and config.seq_len == 1: # use 如果激光雷达序列长度和整体序列长度均为1，禁用与速度和制动相关的损失
    config.detailed_loss_weights['loss_velocity'] = 0.0 # 表明在这种简化场景下，这些预测可能不相关或无法有效学习
    config.detailed_loss_weights['loss_brake'] = 0.0
  ...
  else: # 未启用多任务权重学习，则对所有损失权重进行归一化处理
    # Normalize loss weights.
    factor = 1.0 / sum(config.detailed_loss_weights.values()) # 计算所有损失权重的总和 
    for k in config.detailed_loss_weights: # 将每个权重按比例缩放 保证损失在优化时具有统一的比例，防止某些任务的权重过大或过小
      config.detailed_loss_weights[k] = config.detailed_loss_weights[k] * factor

  # Data, configures config. Create before the model
  train_set = CARLA_Data(root=config.train_data, # /team_code/data.py 加载和准备自动驾驶训练/验证数据集
                         config=config,
  ...
  val_set = CARLA_Data(root=config.val_data, config=config, shared_dict=shared_dict, rank=rank) # /team_code/data.py

  if rank == 0:
    print('Target speed weights: ', config.target_speed_weights, flush=True) # 打印配置文件中定义的目标速度和角度的权重 [0.866605263873406, 7.4527377240841775, 1.2281629310898465, 0.5269622904065803]
    print('Angle weights: ', config.angle_weights, flush=True) #  [204.25901201602136, 7.554315623148331, 0.21388916461734406, 5.476446162657503, 207.86684782608697]
  ...
  find_unused_parameters = True # ori: False
  if config.use_plant:
...
class Engine(object):
  ...
  def train(self):
    ...
    # my add
    sel_p = create_config("/home/wei/carla_garage/team_code/my_config/my_config.yml", None) # 参数文件
    
    loss_weights = sel_p['loss_kwargs']['loss_weights'] # 原始的权重
    criterion = MTLoss_affinity(sel_p, sel_p.TASKS.NAMES, None, loss_weights).cuda()
    
    task_affinity = form_task_affinity(sel_p)
    ...
    # my add 
    self.detailed_loss_weights['loss_bbox'] = 0.1 / 5
    ...
    # Train loop
    print("trainset", len((self.dataloader_train)))
    for i, data in enumerate(tqdm(self.dataloader_train, disable=self.rank != 0)):
        
        # my add      
        if iter_count <= sel_p['prepos']: train_group = list(task_affinity.group.keys()); random.shuffle(train_group)
        ...
        for group in train_group: # ['group_0', 'group_1', 'group_2', 'group_3']
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=bool(self.config.use_amp)):
                ...
                loss_dict = criterion(losses, tasks=sel_p.TASKS.NAMES)
                
                self.optimizer.zero_grad()
                self.scaler.scale(loss_dict[group]).backward()
