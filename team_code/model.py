...
class LidarCenterNet(nn.Module):
    ...
    if self.config.use_controller_input_prediction or self.config.use_wp_gru:
      if self.config.transformer_decoder_join:
        ...
        if self.config.use_controller_input_prediction:
          ...
          self.checkpoint_query = nn.Parameter( # checkpoint_query 是一个可学习的参数，它用于在模型中作为起始点或查询向量。
              torch.zeros(1, self.config.predict_checkpoint_len + 1, self.config.gru_input_size)) # predict_checkpoint_len需要预测的路径点数量 gru_input_sizeGRU 模型的输入维度 + 1 是因为还需要一个目标速度的额外 token（在后面预测中用到）
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command): # 前视图像、LiDAR数据、目标点位置、自车速度、控制命令
    bs = rgb.shape[0] # batch size

    if self.config.backbone == 'transFuser':# 经过自顶向下处理后的 LiDAR 特征图,图像特征与 LiDAR 特征经过融合后的输出,融合后的图像特征
      bev_feature_grid, fused_features, image_feature_grid = self.backbone(rgb, lidar_bev) # /team_code/transfuser.py 将图像和 LiDAR 的特征通过 Transformer 融合
      # dwdwdw
    ...
    if self.config.use_wp_gru or self.config.use_controller_input_prediction: # 基于 GRU 的路径点预测 或 控制器输入预测
      if self.config.transformer_decoder_join:
        fused_features = self.change_channel(fused_features) # [bs,256,8,8]
        fused_features = fused_features + self.encoder_pos_encoding(fused_features) # 935行 位置编码  [bs,256,8,8]
        fused_features = torch.flatten(fused_features, start_dim=2) # [bs,256,8,8]->[bs,256,64]
        if self.config.tp_attention:
          num_pixel_tokens = fused_features.shape[2]

      # Concatenate extra sensor information
      if self.extra_sensors:
        extra_sensors = [] # 存储从传感器提取的特征，最终会拼接到主特征上
        if self.config.use_velocity: # 是否使用自车速度作为传感器输入
          extra_sensors.append(self.velocity_normalization(ego_vel)) # 自车速度归一化处理后加入到 extra_sensors 列表中
        if self.config.use_discrete_command: # 是否使用离散控制命令作为额外输入
          extra_sensors.append(command) # 一个离散值，通常代表目标控制命令（如转向、加速等）
        extra_sensors = torch.cat(extra_sensors, axis=1) # [2,7] 将收集到的额外传感器特征在通道维度上拼接成一个张量
        extra_sensors = self.extra_sensor_encoder(extra_sensors) # [2,256] 两个线性层

        if self.config.transformer_decoder_join:
          extra_sensors = extra_sensors + self.extra_sensor_pos_embed.repeat(bs, 1) # [2,256] 给额外的传感器特征添加位置编码
          fused_features = torch.cat((fused_features, extra_sensors.unsqueeze(2)), axis=2) # [bs,256,65] （64+1）加上额外信息后的融合特征
        else:
          fused_features = torch.cat((fused_features, extra_sensors), axis=1)

      if self.config.transformer_decoder_join:
        fused_features = torch.permute(fused_features, (0, 2, 1)) # [bs,65,256]
        ...
        if self.config.use_controller_input_prediction: # 使用控制器输入预测
          ...
          else: # self.checkpoint_query 是一个查询向量，通常用于在 Transformer 中向模型提供一些固定的信息，比如起始位置、路径点的参考信息等
            joined_checkpoint_features = self.join(self.checkpoint_query.repeat(bs, 1, 1), fused_features) # [bs, 11, 256] 三个线性层＋relu
  ...
  def compute_loss(self, pred_wp, pred_target_speed, pred_checkpoint, pred_semantic, pred_bev_semantic, pred_depth,
    ...
    if self.config.detect_boxes:
      ...
      # my add
      loss_bbox_sum = sum(loss_bbox.values())
      loss.update({'loss_bbox': loss_bbox_sum})
...
class PositionEmbeddingSine(nn.Module):
  ...
  def forward(self, tensor): # 图像特征与 LiDAR 特征经过融合后的输出
    x = tensor  # [bs,256,8,8]
    bs, _, h, w = x.shape
    not_mask = torch.ones((bs, h, w), device=x.device) # [bs,8,8] 全为1的张量，用来生成位置网格
    y_embed = not_mask.cumsum(1, dtype=torch.float32) # [bs,8,8] 每个位置生成唯一的索引
    x_embed = not_mask.cumsum(2, dtype=torch.float32) # [bs,8,8]
