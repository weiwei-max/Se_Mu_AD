...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
  ...
  def __init__(self,
               ...
               rank=0):
    self.config = config
    assert config.img_seq_len == 1 # 确保图像序列长度为 1，表示数据不处理时间序列信息
    ...
    for sub_root in tqdm(root, file=sys.stdout, disable=rank != 0): # tqdm 是一个进度条工具，用于显示循环的执行进度
      ...
      for route in routes:
        ...
        for seq in range(config.skip_first, num_seq - self.config.pred_len - self.config.seq_len):
          ...
          for idx in range(self.config.seq_len): # 加载每一帧数据对应的图像、语义图像、LiDAR 点云数据、深度图等
